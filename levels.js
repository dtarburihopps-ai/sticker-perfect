...
//
//   места ничьи (group без sticker) — место принимает любой товар, пока
//   его группа пустая. Первый вставший закрепляет группу за собой, и чужой
//   товар туда уже не встанет: полки в шкафу.
//
...
//     python tools/shelf.py      шкаф с продуктами
//
...
const LEVELS = {
  ...
  // --- Уровень 6: шкаф с продуктами ---
  //
  // Шесть одинаковых полок, на каждой ровно три места, и шесть видов товара
  // по три штуки: восемнадцать товаров на восемнадцать мест, всё сходится
  // ровно.
  //
  // Пятое устройство уровня. Места посчитаны, как в холодильнике, но ничьи:
  // товар идёт на любую полку, пока она пустая. Как только на полку встал
  // первый — она закрепляется за ним, и чужой товар туда уже не поставить.
  //
  // Затравок нет и подсказок нет: правило игрок узнаёт с первого отказа,
  // а объяснять его словами не приходится.
  //
  // В тупик уровень загнать нельзя: видов шесть и полок шесть, каждому
  // виду достанется своя, в каком бы порядке их ни ставили. Переигровка
  // начинается с пустого шкафа — полки снова ничьи.
  //
  //     python tools/shelf.py     шкаф, товары и координаты мест
  shelf: {
    background: 'web/shelf.webp',
    wall: '#EBDDC9',

    // Полоса выше обычной, как у карандашей: товар высокий и узкий,
    // при обычной высоте шесть штук ужались бы в ниточки.
    trayHeight: 150,
    trayVisible: 6,

    // Габарит у всех шести один и тот же, и это не случайность: полка
    // принимает любой товар, а товар разного роста выглядел бы ошибкой.
    stickers: {
      soda:    { image: 'web/shelf-soda.webp',    width: 0.1230, height: 0.2349 },
      milk:    { image: 'web/shelf-milk.webp',    width: 0.1230, height: 0.2349 },
      chips:   { image: 'web/shelf-chips.webp',   width: 0.1230, height: 0.2349 },
      oil:     { image: 'web/shelf-oil.webp',     width: 0.1230, height: 0.2349 },
      juice:   { image: 'web/shelf-juice.webp',   width: 0.1230, height: 0.2349 },
      cookies: { image: 'web/shelf-cookies.webp', width: 0.1230, height: 0.2349 }
    },

    // Размер места. Товара за местом не закреплено, а подойти может любой —
    // и все они одного габарита.
    slotSize: { width: 0.1230, height: 0.2349 },

    // Шесть полок по три места. group — это полка: заранее она ни за кем
    // не закреплена и достаётся тому, кто встал первым.
    slots: [
      { id: 'shelf1-1', group: 'shelf1', x: 0.0996, bottom: 0.3373 },
      { id: 'shelf1-2', group: 'shelf1', x: 0.2227, bottom: 0.3373 },
      { id: 'shelf1-3', group: 'shelf1', x: 0.3457, bottom: 0.3373 },

      { id: 'shelf2-1', group: 'shelf2', x: 0.5303, bottom: 0.3373 },
      { id: 'shelf2-2', group: 'shelf2', x: 0.6533, bottom: 0.3373 },
      { id: 'shelf2-3', group: 'shelf2', x: 0.7764, bottom: 0.3373 },

      { id: 'shelf3-1', group: 'shelf3', x: 0.0996, bottom: 0.6151 },
      { id: 'shelf3-2', group: 'shelf3', x: 0.2227, bottom: 0.6151 },
      { id: 'shelf3-3', group: 'shelf3', x: 0.3457, bottom: 0.6151 },

      { id: 'shelf4-1', group: 'shelf4', x: 0.5303, bottom: 0.6151 },
      { id: 'shelf4-2', group: 'shelf4', x: 0.6533, bottom: 0.6151 },
      { id: 'shelf4-3', group: 'shelf4', x: 0.7764, bottom: 0.6151 },

      { id: 'shelf5-1', group: 'shelf5', x: 0.0996, bottom: 0.9012 },
      { id: 'shelf5-2', group: 'shelf5', x: 0.2227, bottom: 0.9012 },
      { id: 'shelf5-3', group: 'shelf5', x: 0.3457, bottom: 0.9012 },

      { id: 'shelf6-1', group: 'shelf6', x: 0.5303, bottom: 0.9012 },
      { id: 'shelf6-2', group: 'shelf6', x: 0.6533, bottom: 0.9012 },
      { id: 'shelf6-3', group: 'shelf6', x: 0.7764, bottom: 0.9012 }
    ],

    // В полосе вперемешку и без двух одинаковых подряд: иначе игрок
    // выкладывал бы их кучками, не выбирая полку.
    tray: [
      'soda', 'milk', 'chips', 'oil', 'juice', 'cookies',
      'milk', 'soda', 'oil', 'chips', 'cookies', 'juice',
      'soda', 'cookies', 'milk', 'juice', 'chips', 'oil'
    ]
  }
};

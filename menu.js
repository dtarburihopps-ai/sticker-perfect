...
function showMenu() {
  ...
  // Прогресс мог сохраниться в облаке телеграма — например, игрок чистил
  // данные или зашёл с другого телефона. Ждать облако мы не стали: альбом
  // уже открыт. Придёт ответ, и если там больше — карточки откроются
  // прямо на глазах. Это читается как «подтянулось», а не как сбой.
  restoreProgress(function () {
    menuPage = Math.floor((openedCount() - 1) / PER_PAGE);
    drawMenu();
    log('Альбом перерисован: открыто уровней —', openedCount());
  });
